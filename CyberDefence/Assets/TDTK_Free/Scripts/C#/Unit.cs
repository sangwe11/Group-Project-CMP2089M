...
public class UnitAttribute{
	...
	public void Init(Transform transform){
		...
		if(overlayHP!=null) {
			...
			overlayRendererH=overlayHP.GetComponent<Renderer>();
		...
		if(!alwaysShowOverlay){
			...
			if(overlayBase && !neverShowBase){
				if(overlayHP && overlayRendererH.enabled){
					overlayBase.GetComponent<Renderer>().enabled=true;
				}
				else{
					if(HP>=fullHP) overlayBase.GetComponent<Renderer>().enabled=false;
				}
			}
